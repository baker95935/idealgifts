...
//	}
    ...
    public function insert_by_post_param($table, $column, $post){
        $param = explode(',',$column);
        foreach ($param as $val){
            if(array_key_exists($val, $post)){
            $column_arr[] = $val;
                     $value_arr[$val] = "'".$post[$val]."'";
            }
        }
    ...
    public function update_by_post_param($table, $column, $post, $condition){
        $param = explode(',',$column);
        $mod_content = '';
        foreach ($param as $val){
            if(array_key_exists($val, $post)){
                     $column_arr[] = $val.'='."'".$post[$val]."'";
            }
        }
        $mod_content = implode(',', $column_arr);
       
        return $this->update($table, $mod_content, $condition);
    }
    ...
    public function get_count($table,$pk = 'id',$condition){
         $condition = $condition ? ' Where ' . $condition : NULL;
        if($table){
           if($result = $this->query("select count(`$pk`) as sum from $table $condition")){
...
//function __destruct();
    ...
    function get_array($result){
        $data = NULL;
        while ($rs = $this->fetch_assoc($result)){
            $data[] = $rs;
        }
        return $data;
    }
    function get_one_object($result){
        $data = NULL;
        while ($rs = $this->fetch_assoc($result)){
            $data = $rs;
        }
    ...
    function mysql_transaction($function){
