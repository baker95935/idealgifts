...
class Captcha { //class start
    ...
    public function create($length = 4, $param = array()) {
        ...
        $width = isset($param['width']) ? $param['width'] : 18; //文字宽度
        $height = isset($param['height']) ? $param['height'] : 21; //文字高度
        $pnum = isset($param['pnum']) ? $param['pnum'] : 100; //干扰象素个数
        $lnum = isset($param['lnum']) ? $param['lnum'] : 0; //干扰线条数
