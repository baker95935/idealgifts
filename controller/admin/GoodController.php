...
class GoodController extends CommonController {
    ...
    private function get_all_goods($page) {
        $model = $this->getModel();
        $db = $model->getDb();
        ...
        $page = new Page($this->get_goods_count(), Application::$_config['page']['page_size']);
        $this->setValue('data', $data);
        $this->setValue('page', $page->showpage());
    }

    private function get_goods_count() {
        $model = $this->getModel();
        $db = $this->getDb();
        return $db->get_count($model->table('good'), 'good_id', '');
    }
